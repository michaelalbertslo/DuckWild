...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "mode",
        choices=["photo", "video", "server"],  # <-- ADD server
        help="photo, video, or server (stdin CAPTURE commands)",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--wait-for-gpio",
        action="store_true",
        help="wait for GPIO pin HIGH before capturing (photo mode only)",
    )
    parser.add_argument(
        "--gpio-pin",
        type=int,
...
def _gpio_setup_input(pin: int) -> None:
    if GPIO is None:
        raise RuntimeError("RPi.GPIO is required for GPIO triggers.")
...
def wait_for_gpio_high(pin: int) -> None:
    ...
    try:
        ...
        GPIO.cleanup(pin)
    print("GPIO trigger. Capturing photo...")
...
def main() -> None:
    ...
    try:
        ...
        if args.mode == "photo":
            ...
            capture_photo_fast(picam2, output, width=args.width, height=args.height, warmup_ms=args.warmup_ms)
